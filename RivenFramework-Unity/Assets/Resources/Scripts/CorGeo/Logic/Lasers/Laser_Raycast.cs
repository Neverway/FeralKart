...
public class Laser_Raycast : MonoBehaviour
{
    ...
    private void DrawReflectionPattern (Vector3 position, Vector3 direction, int reflectionsRemaining, bool bounce = true)
    {
        ...
        if (didHit)
        {
            ...
            if (hit.collider.gameObject.TryGetComponent<CorGeo_SliceableMesh> (out var slicer))
            {
            ...
            else if (hit.collider.gameObject.GetComponentInParent<Laser_Detector>())
            {
                bounce = false;
                OnHit(hit);
            }
    ...
    {
        var detector = hit.collider.gameObject.GetComponentInParent<Laser_Detector>();
        if (detector)
        {
            detector.OnHit ();
        }
