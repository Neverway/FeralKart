...
/*
public class VoxComputeManager : MonoBehaviour
{
    ...
    {
        xThreads = VoxWorldManager.WorldSettings.containerSize / 8 + 1;
        yThreads = VoxWorldManager.WorldSettings.maxHeight / 8;
        
        noiseShader.SetInt("containerSizeX", VoxWorldManager.WorldSettings.containerSize);
        noiseShader.SetInt("containerSizeY", VoxWorldManager.WorldSettings.maxHeight);
        
        noiseShader.SetBool("generateCaves", true);
        noiseShader.SetBool("forceFloors", true);
        
        noiseShader.SetInt("maxHeight", VoxWorldManager.WorldSettings.maxHeight);
        noiseShader.SetInt("SeaLevel", 40);
    ...
    public NoiseBuffer CreateNewNoiseBuffer(bool enqueue)
    {
    ...
    public void GenerateVoxelData(ref VoxContainer cont, Vector3 pos)
    {
        ...
        noiseShader.SetVector("seedOffset", Vector4.zero);
        
        noiseShader.Dispatch(0, xThreads, yThreads, xThreads);

        AsyncGPUReadback.Request(cont.data.noiseBuffer, (callback) =>
        {
            callback.GetData<Voxel>(0).CopyTo(VoxWorldManager.Instance.container.data.voxelArray.array);
            VoxWorldManager.Instance.container.RenderMesh();
        });
    }
    ...
    private static VoxComputeManager _instance;

    public static VoxComputeManager Instance
    {
        get
        {
            if (_instance == null)
            {
                _instance = FindObjectOfType<VoxComputeManager>();
            }
