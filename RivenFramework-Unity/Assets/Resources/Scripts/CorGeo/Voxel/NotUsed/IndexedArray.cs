...
public class IndexedArray<T> where T : struct
{
    ...
    public IndexedArray()
    {
        Create(VoxWorldManager.WorldSettings.containerSize, VoxWorldManager.WorldSettings.maxHeight);
    }
    ...
    private int IndexFromCoord(Vector3 index)
    {
    ...
    public T this[Vector3 coord]
    {
        get
        {
            if (coord.x < 0 || coord.x > size.x || coord.y < 0 || coord.y > size.y || coord.z < 0 || coord.z > size.x)
            {
