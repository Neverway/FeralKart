...
{
    [MenuItem("GameObject/Assets/Sort selected to 'Area'", false, 0)]
    ...
    [MenuItem("GameObject/Assets/Rename & sort selected", false, 0)]
    private static void RenameAndSortSelected()
    {
        RenameSelectedToPrefabName();
        RenameSelectedTextToMatchContent();
        SortSelectedAlphabetically();
    }
    
    [MenuItem("GameObject/Assets/Rename selected to prefab name", false, 0)]
    ...
    [MenuItem("GameObject/Assets/Rename selected text to match content", false, 0)]
    private static void RenameSelectedTextToMatchContent()
    {
        foreach (var obj in Selection.gameObjects)
        {
            TMP_Text textObject = obj.GetComponent<TMP_Text>();

            if (textObject)
            {
                textObject.name = $"Text ({textObject.text})";
                continue;
            }
            
            Text genericTextObject = obj.GetComponent<Text>();

            if (genericTextObject)
            {
                genericTextObject.name = $"[Text] + ({textObject.text})";
                continue;
            }
        }
    }
    
    [MenuItem("GameObject/Assets/Sort selected alphabetically", false, 0)]
    private static void SortSelectedAlphabetically()
    {
