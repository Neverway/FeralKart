...
namespace Neverway.Framework.Cartographer
{
    public class WB_LevelEditor : MonoBehaviour
    {
        ...
        [SerializeField] private Button[] fileButtons,
            ...
            layerButtons,
            sublayerButtons,
            layerVisibilityButtons;

        [SerializeField] private Button clearHotbarButton, toggleInventoryButton;

        [SerializeField]
        private GameObject tileCursor, inspectionIndicator, shapePainterIndicator, shapePainterIndicatorPrefab;
        ...
        private void InitializeEventListeners()
        {
            ...
            if (!projectData) projectData = FindObjectOfType<ProjectDatabase>();
            ...
            {
                int index = i; // Capturing the variable in the lambda expression
                layerVisibilityButtons[i].onClick.AddListener(() =>
                {
                    IsLayerVisible[index] = !IsLayerVisible[index];
                });
            }
        }
        ...
        private void SetToolIconColors(Image _paintImage, Image _inspectImage)
        {
            ...
            {
                tileCursor.transform.position = new Vector3((float)MathF.Floor(cursorPos.x / 1) * 1 + cursorOffset,
                    (float)MathF.Floor(cursorPos.y / 1) * 1 + cursorOffset, 0);
            }
            else
            {
                tileCursor.transform.position = new Vector3((float)MathF.Round(cursorPos.x / 1) * 1 + cursorOffset,
                    (float)MathF.Round(cursorPos.y / 1) * 1 + cursorOffset, 0);
            }
        ...
        private Color GetCursorColor()
        {
            if (currentTool == "paint")
            {
                if (projectData.GetTileFromMemory(hotBarTileID[currentHotBarIndex]) &&
                    !hotBarTileID[currentHotBarIndex].Contains("Collision"))
                ...
                else if (projectData.GetTileFromMemory(hotBarTileID[currentHotBarIndex]) &&
                         hotBarTileID[currentHotBarIndex].Contains("Collision"))
        ...
        private void UpdateLayerSelection()
        {
            ...
            sublayerButtons[currentSublayer].transform.GetChild(0).GetComponent<TMP_Text>().color =
                new Color(1, 1, 1, 1);
        ...
        private void UpdateLayerVisibility()
        {
            for (int i = 0; i < IsLayerVisible.Length; i++)
            {
                ...
                {
                    case true:
                        layerVisibilityButtons[i].transform.GetChild(0).GetComponent<Image>().sprite =
                            visibilitySprites[0];
        ...
        private void PlaceTile(Vector3 _position = default)
        {
            ...
            Vector3Int position =
                new Vector3Int((int)MathF.Floor(positionToUse.x), (int)MathF.Floor(positionToUse.y), 0);
        ...
        private void PlaceAsset(Vector3 _position = default)
        {
            ...
            for (int i = 0; i < levelManager.assetsRoot.transform.childCount; i++)
            {
                ...
                if (child.position == new Vector3(MathF.Round(positionToUse.x), MathF.Round(positionToUse.y),
                        child.position.z))
                {
            ...
            GameObject
                asset = projectData
                    .missingObjectFallback; // assign the asset as the missing object until it's overwritten
            if (projectData.GetActorFromMemory(hotBarTileID[currentHotBarIndex]))
            {
                if (projectData.GetActorFromMemory(hotBarTileID[currentHotBarIndex]).AssociatedGameObject)
                    asset = projectData.GetActorFromMemory(hotBarTileID[currentHotBarIndex]).AssociatedGameObject;
            }
            ...
            GameObject assetRef =
                Instantiate(asset, assetPosition, Quaternion.identity, levelManager.assetsRoot.transform);
            assetRef.name = assetRef.name.Replace("(Clone)", "").Trim();

            // Assign the Actor data to the game object
            assetRef.GetComponent<ActorData>().actorId =
                projectData.GetActorFromMemory(hotBarTileID[currentHotBarIndex]).id;
            switch (projectData.GetActorType(hotBarTileID[currentHotBarIndex]))
            {
                ...
                case "item":
                    ...
                    break;
                case "character":
                    asset.GetComponent<Pawn>()
                        .SetPawnDefaultState(projectData.GetCharacterFromMemory(hotBarTileID[currentHotBarIndex]));
                    break;
            }
        ...
        private void Inspect()
        {
            ...
            foreach (Transform child in levelManager.assetsRoot.transform)
            {
                ...
                float distance = Vector2.Distance(new Vector2(cursorPos.x, cursorPos.y),
        ...
        private void EraseAsset(Vector3 _position = default)
        {
            ...
            for (int i = 0; i < levelManager.assetsRoot.transform.childCount; i++)
            {
                ...
                float distance = Vector2.Distance(new Vector2(positionToUse.x, positionToUse.y),
                    new Vector2(child.position.x, child.position.y));
