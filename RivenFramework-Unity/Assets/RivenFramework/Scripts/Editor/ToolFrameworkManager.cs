    ...
    public class ToolFrameworkManager : EditorWindow
    {
        ...
        private void Home()
        {
            EditorGUILayout.HelpBox(
                "Welcome to the Riven Framework Manager! This window will allow you to access a variety of tools for setting up your Riven Framework project!",
                MessageType.None);
            GUILayout.Space(10);
        ...
        private void ProjectSetup()
        {
            ...
            if (GetPrefabAtPath("GameInstance", SystemObjectsFolder) is not null)
            {
                GameInstance gameInstance =
                    GetPrefabAtPath("GameInstance", SystemObjectsFolder).GetComponent<GameInstance>();
                ...
                {
                    Debug.LogError("GameInstance component not found on the prefab.");
                }

                ApplicationSettings applicationSettings = GetPrefabAtPath("GameInstance", SystemObjectsFolder)
                    .GetComponent<ApplicationSettings>();
        ...
        {
            EditorGUILayout.HelpBox(
                "Whoops, it looks like you reached a dead end! This section may not be complete or an issue may have occured. Please report this to a developer.",
                MessageType.Error);
            GUILayout.Space(10);
            if (GUILayout.Button("Back"))
            {
