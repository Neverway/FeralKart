...
namespace Neverway.Framework.LogicSystem
{
    public class Volume : LogicComponent
    {
        ...
        [Tooltip(
            "If enabled, physics props that are being held won't be effected by the volume (This is used for things like wind boxes)")]
        public bool ignoreHeldObjects = true;

        [Tooltip(
            "If enabled, will remove objects from the list of objects in the volume when that object becomes disabled")]
        ...
        protected void OnTriggerEnter(Collider _other)
        {
            ...
            {/*
                ...
                {
                    if (_other.gameObject.GetComponent<Object_Grabbable>().isHeld)
                    {
                        return;
                    }
                }*/
